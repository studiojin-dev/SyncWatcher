# ADR-20260307-0011-PRG: Exclusion defaults consolidation into a program set
Status: Accepted
Date: 2026-03-07
Tags: sync-task, exclusion, migration, config-store, ux, macos, tauri
TL;DR: Replace per-ecosystem built-in exclusion defaults with one `program` built-in set, automatically migrate legacy built-in IDs in stored exclusion sets and tasks, and remove `.gitignore` from the `git` built-in set.
...
5. Automatically migrate stored config on load:
