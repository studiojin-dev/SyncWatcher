...
mod integration_tests {
    ...
    use crate::{
        ...
        record_runtime_validation_issue, runtime_validation_issue_log_message,
        ...
        runtime_get_state_internal, select_runtime_dispatch_candidate,
        ...
        AppState, CancelOperationType, DataUnitSystem, DryRunLiveState,
        RuntimeAutoUnmountDecision, RuntimeConfigPayload, RuntimeExclusionSet,
    ...
    #[test]
    fn test_select_runtime_dispatch_candidate_blocks_downstream_while_upstream_is_queued() {
        ...
        let runtime_config = RuntimeConfigPayload {
            tasks: vec![
                build_runtime_task_with_paths("upstream", "/src/a", "/chain/root", true),
                build_runtime_task_with_paths("downstream", "/chain/root/inbound", "/dst/b", true),
                build_runtime_task_with_paths("independent", "/other/source", "/other/target", true),
            ],
            ..Default::default()
        };
        ...
        assert_eq!(selection.task_id.as_deref(), Some("upstream"));
        assert_eq!(selection.next_ready_in, None);
    }

    #[test]
    fn test_select_runtime_dispatch_candidate_waits_for_settle_window() {
        let queue = VecDeque::from(["downstream".to_string()]);
        let queued: HashSet<String> = queue.iter().cloned().collect();
        let syncing = HashSet::new();
        let now = Instant::now();
        let mut settle_until = HashMap::new();
        settle_until.insert("downstream".to_string(), now + Duration::from_millis(450));
        let runtime_config = RuntimeConfigPayload {
            tasks: vec![build_runtime_task_with_paths(
                "downstream",
                "/chain/root/inbound",
                "/dst/b",
                true,
        ...
        assert_eq!(selection.next_ready_in, Some(Duration::from_millis(450)));
    }

    #[test]
    fn test_select_runtime_dispatch_candidate_prefers_ready_independent_task() {
        let queue = VecDeque::from(["downstream".to_string(), "independent".to_string()]);
        let queued: HashSet<String> = queue.iter().cloned().collect();
        let syncing: HashSet<String> = HashSet::from(["upstream".to_string()]);
        let settle_until = HashMap::new();
        let runtime_config = RuntimeConfigPayload {
            tasks: vec![
                build_runtime_task_with_paths("upstream", "/src/a", "/chain/root", true),
        ...
        assert_eq!(selection.task_id.as_deref(), Some("independent"));
    }
