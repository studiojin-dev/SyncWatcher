...
function SyncTasksView({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const resolvedSourceUuidType: SourceUuidType | undefined =
      sourceType === 'uuid'
        ? sourceUuidType ||
          inferUuidTypeFromVolumes(sourceUuid, volumes) ||
          'disk'
        : undefined;
    ...
    const taskData = {
      ...
      sourceUuidType:
        sourceType === 'uuid' ? resolvedSourceUuidType : undefined,
      sourceSubPath:
        sourceType === 'uuid' ? normalizedSourceSubPath : undefined,
    };
                    ...
                    <div className="space-y-2">
                      <Select
                        ...
                        onChange={(value) => {
                          ...
                          const option = uuidSourceOptions.find(
                      ...
                      <button
                        ...
                        onClick={() =>
                          setSubView({
                            ...
                            source: task.source,
                            target: task.target,
                            excludePatterns: getPatternsForSets(
                              task.exclusionSets || [],
                            ),
                          })
                        }
                          ...
                          const currentFilePercent =
                            currentFileSize > 0
                              ? Math.min(
                                  100,
                                  Math.round(
                                    ((progress.currentFileBytesCopied || 0) /
                                      currentFileSize) *
                                      100,
                                  ),
                                )
